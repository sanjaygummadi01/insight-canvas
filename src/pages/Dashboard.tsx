      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {metrics.map((metric, i) => (
            <MetricCard key={metric.title} data={metric} delay={i * 100} />
          ))}
        </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="p-4 bg-muted/50 rounded-lg">
              ...
              <p className="text-2xl font-bold text-foreground">
                ${filteredData.length > 0 
                  ? Math.round(filteredData.reduce((a, b) => a + b.sales, 0) / filteredData.length).toLocaleString()
                  : 0}
              </p>
            </div>
            <div className="p-4 bg-muted/50 rounded-lg">
              ...
              <p className="text-sm font-medium text-foreground">
                {filteredData.length > 0
                  ? filteredData.reduce((max, row) => row.revenue > max.revenue ? row : max, filteredData[0]).date
                  : 'No data'}
              </p>
            </div>
          </div>
