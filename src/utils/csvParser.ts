...
export const parseCSV = (content: string): DataRow[] => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    headers.forEach((header, index) => {
      ...
      if (header === 'date') {
        row.date = value;
      } else if (['sales', 'revenue', 'users', 'growth'].includes(header)) {
...
export const generateCSV = (data: DataRow[]): string => {
...
export const generatePowerBICSV = (data: DataRow[]): string => {
  if (data.length === 0) return '';
...
export const generateJSON = (data: DataRow[]): string => {
