...
export const GrowthIndicator: React.FC<GrowthIndicatorProps> = ({ data, title }) => {
        ...
        <div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-foreground">
              {chartData.trendPercentage.toFixed(1)}%
            </span>
            <span 
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {chartData.isPositive ? 'Growth' : 'Decline'} over period
          </p>
        </div>
        ...
        <svg width="200" height="60" className="overflow-visible">
          <defs>
            <linearGradient id="sparklineGradient" x1="0" y1="0" x2="1" y2="0">
              <stop 
                offset="0%" 
                stopColor={chartData.isPositive ? 'hsl(var(--success))' : 'hsl(var(--destructive))'} 
                stopOpacity={0.3} 
              />
              <stop 
                offset="100%" 
          ...
          <circle
            cx={200}
            cy={60 - ((chartData.currentValue - Math.min(...data.slice(-30).map(d => d.value))) / 
              (Math.max(...data.slice(-30).map(d => d.value)) - Math.min(...data.slice(-30).map(d => d.value)) || 1)) * 60}
            r={4}
