import { DataRow, DateRange, ChartDataPoint, MetricCardData } from '@/types/analytics';

export const filterDataByDateRange = (data: DataRow[], dateRange: DateRange): DataRow[] => {
...
export const calculateMetrics = (data: DataRow[]): MetricCardData[] => {
  if (data.length === 0) {
    return [
  ...
  // Calculate changes (mock comparison)
  ...
  const usersChange = firstHalfUsers > 0 
  ...
  return [
    ...
    },
    {
      ...
      change: parseFloat(avgGrowth.toFixed(1)),
      changeLabel: 'average',
      icon: 'growth',
    },
  ];
...
export const prepareLineChartData = (data: DataRow[]): ChartDataPoint[] => {
  return data.map(row => ({
...
export const prepareBarChartData = (data: DataRow[]): ChartDataPoint[] => {
